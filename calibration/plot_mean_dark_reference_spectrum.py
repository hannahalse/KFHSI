...
DEFAULT_IMAGE_DIR = BASE_DIR / "darkReference"
...
def parse_snapshot_datetime(image_path: Path):
    ...
    snapshot_camera_hsi_snapshot_13.5.2026, 13_49_05.png
    """
...
def plot_dark_reference_spectra(
    wavs_nm,
    spectra,
    title: str,
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract spectra from all dark-reference PNGs, compute their mean, and plot them."
    )
    parser.add_argument(
        "--image-dir",
        default=str(DEFAULT_IMAGE_DIR),
        help=f"Folder containing dark-reference PNGs (default: {DEFAULT_IMAGE_DIR})",
    )
    parser.add_argument(
        "--no-flip-x",
        action="store_true",
    ...
    mean_spectrum = np.mean(spectra, axis=0)
    print(f"Loaded {len(image_paths)} dark-reference images from {image_dir}")
    ...
    plot_dark_reference_spectra(
        wavs_nm,
        spectra,
        title=f"Dark-reference spectra and mean ({len(image_paths)} images)",
